...
TEST(NTTTest, RoundtripAllSecurityLevels) {
    ...
    check_ntt_roundtrip_for_params(
        RLWE::getParameterSet(SecurityLevel::TEST_TINY), 0xA1B2C3D4ULL);
    check_ntt_roundtrip_for_params(
        RLWE::getParameterSet(SecurityLevel::TEST_SMALL), 0xBEEF1234ULL);

    // Kyber-like 128-bit, NTT-friendly parameters
    check_ntt_roundtrip_for_params(
        RLWE::getParameterSet(SecurityLevel::KYBER512), 0x12345678ULL);

    // Moderate and High security levels
    check_ntt_roundtrip_for_params(
        RLWE::getParameterSet(SecurityLevel::MODERATE), 0xCAFEBABEULL);
    check_ntt_roundtrip_for_params(
        RLWE::getParameterSet(SecurityLevel::HIGH), 0xDEADBEEFULL);
}
